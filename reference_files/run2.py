...
from aerodynamics.group_aero import AeroGroup
...
comp.add_output('mean_chord',val=0.1, units='m')
comp.add_output('alpha',val=2, units='deg')
comp.add_output('wing_area',val=17346,units='m**2')
prob.model.add_subsystem('opt_input_comp', comp, promotes=['*'])


comp = IndepVarComp()
comp.add_output('CLa',val=0.1)
comp.add_output('CL0',val=0.1)
comp.add_output('CDv',val=0.1)
comp.add_output('CD0',val=0.1)
...
prob.model.add_subsystem('aero_group', group, promotes=['*'])
